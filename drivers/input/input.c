...
static inline void input_start_autorepeat(struct input_dev *dev, int code)
{
	if (test_bit(EV_REP, dev->evbit) && dev->rep[REP_PERIOD] &&
	    dev->rep[REP_DELAY] && dev->timer.data) {
		dev->repeat_key = code;
		mod_timer(&dev->timer,
...
static inline void input_stop_autorepeat(struct input_dev *dev)
{
	del_timer(&dev->timer);
}
			      ...
			      unsigned int count)
{
	...
	for (v = vals; v != vals + count; v++)
		if (v->type == EV_KEY && v->value != 2)
			v->value ? input_start_autorepeat(dev, v->code) :
