...
/**
 * mdss_mdp_get_fb0_ctl() - get MDP control data of a main display (FB0).
 *
 * Tries to acquire the control data which belongs to a main display, usually
 * connected to the first framebuffer. In case the first framebuffer data is
 * not currently available, this function will return NULL pointer. Although
 * this is suboptimal, it isn't concerned as an issue due to a fact that the
 * internal MDP functions can successfully update panel registers even with
 * the absence of control data.  However, if this happens, display won't be
 * updated immediately as mdss_mdp_pp_setup() won't be called.
 */
static struct mdss_mdp_ctl *mdss_mdp_get_fb0_ctl(void)
{
	...
	for (i = 0; i < mdata->nctl; i++) {
		ctl = mdata->ctl_off + i;
		/* We need to setup main (fb0) display only */
		if (ctl && ctl->mfd && ctl->mfd->index == 0)
			return ctl;
	}
...
{
	struct mdss_mdp_ctl *ctl = mdss_mdp_get_fb0_ctl();
	...
	mdss_mdp_pcc_config_kcal(ctl, &pcc_config); /* Cannot fail here */
...
static inline void mdss_mdp_kcal_update_pa(struct kcal_lut_data *lut_data)
{
	...
	mdss_mdp_pa_config_kcal(ctl, &pa_config); /* Cannot fail here */
}
