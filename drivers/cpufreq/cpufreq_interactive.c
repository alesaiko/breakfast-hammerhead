...
struct cpufreq_interactive_cpuinfo {
	...
	spinlock_t target_freq_lock; /* Potects target freq */
 ...
 * this frequency and allow for dynamic changes above.
 */
static unsigned long freq_calc_thresh = 1036800;
 ...
 * frequency.
 */
static unsigned int max_freq_hysteresis = 40000;
...
static void cpufreq_interactive_timer(unsigned long data)
{
	...
	if (new_freq < pcpu->floor_freq &&
	    now - pcpu->floor_validate_time < pcpu->min_sample_time) {
		spin_unlock_irqrestore(&pcpu->target_freq_lock, flags);
		goto rearm;
	}
...
static ssize_t store_target_loads(struct kobject *kobj, struct attribute *attr,
...
static struct global_attr freq_calc_thresh_attr =
	__ATTR(freq_calc_thresh, 0644,
...
static struct global_attr hispeed_freq_attr =
...
static struct global_attr align_windows_attr =
	__ATTR(align_windows, 0644,
...
static struct global_attr go_hispeed_load_attr =
	__ATTR(go_hispeed_load, 0644,
...
static struct global_attr min_sample_time_attr =
	__ATTR(min_sample_time, 0644,
...
static struct global_attr timer_rate_attr =
	__ATTR(timer_rate, 0644,
...
static ssize_t store_timer_slack(struct kobject *kobj, struct attribute *attr,
				 const char *buf, size_t count)
{
	int ntokens;
