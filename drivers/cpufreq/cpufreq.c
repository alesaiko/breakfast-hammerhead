...
static inline u64 get_cpu_idle_time_jiffy(u32 cpu, u64 *wall)
{
	u64 idle_time, cur_wall_time, busy_time;
...
u64 get_cpu_idle_time(u32 cpu, u64 *wall, int io_is_busy)
{
	u64 idle_time;

	idle_time = get_cpu_idle_time_us(cpu, io_is_busy ? wall : NULL);
	if (idle_time == -1ULL)
