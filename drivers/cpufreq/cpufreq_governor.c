			...
			u32 *max_load_freq)
{
	...
	for_each_cpu(cpu, policy->cpus) {
		...
		cur_idle_time =
			get_cpu_idle_time(cpu, &cur_wall_time, io_is_busy);

		/* Get common dbs for current cpu */
			 ...
			 struct cpufreq_policy *policy, u32 event)
{
	...
	struct cpu_dbs_common_info *cdbs, *j_cdbs;
	struct od_dbs_tuners *od_tuners;
	struct cs_dbs_tuners *cs_tuners;
	u32 sampling_rate = 0;
	int cpu, ret, j;

	cpu = policy->cpu;
	cdbs = dbs_data->get_cpu_cdbs(cpu);
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			switch (dbs_data->governor) {
			case GOV_ONDEMAND: {
				struct od_cpu_dbs_info_s *j_od_dbs_info;
