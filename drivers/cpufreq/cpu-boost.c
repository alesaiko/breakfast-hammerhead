...
/**
 * get_cpu_sync_info() - get synchronization information of a cpu.
 * @cpu: cpu number to get sync_info of.
 */
			       ...
			       unsigned long val, void *data)
{
	...
	u32 b_min = s->boost_min, ib_min = s->input_boost_min, min = 0;

	switch (val) {
	case CPUFREQ_ADJUST:
		/* We should boost either to boost_min or input_boost_min */
		if (!b_min && !ib_min)
			break;

		/*
		 * Override minimum frequency of a policy, but never exceed
		 * policy->max, as we can conflict with user-applied frequency
		 * bound or interrupt thermal throttling process.
		 */
		min = clamp(min, max(b_min, ib_min), policy->max);
		cpufreq_verify_within_limits(policy, min, UINT_MAX);
		break;
	case CPUFREQ_START:
		/* Run a synchronization thread on a hotplugged cpu */
...
static void do_boost_rem(struct work_struct *work)
{
	...
	/* Reset boost frequency of an affected cpu */
	s->boost_min = 0;
...
static int boost_mig_sync_thread(void *data)
{
	...
	while (1) {
		...
		/* Set an accounted boost frequency */
...
static void do_input_boost_rem(struct work_struct *work)
{
	...
	/* Reset input boost frequency for all possible cpus */
	for_each_possible_cpu(cpu) {
		struct cpu_sync *s = get_cpu_sync_info(cpu);
		s->input_boost_min = 0;
	}
...
static void do_input_boost(struct work_struct *work)
{
	...
	/* Setup input boost frequency and apply it to all online cpus */
	for_each_possible_cpu(cpu) {
		struct cpu_sync *s = get_cpu_sync_info(cpu);
	...
	/* Input boosting will be reset after input_boost_ms */
				 ...
				 int value)
{
	...
	/* Return early if boosting is disabled */
	...
	/* Return early if boosting is still in progress */
