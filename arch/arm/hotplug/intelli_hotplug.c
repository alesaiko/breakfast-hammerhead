/*
 * Copyright (c) 2013-2017, Paul Reioux <reioux@gmail.com>
 *
...
static unsigned int full_mode_profile = 0; /* balanced profile */
...
static unsigned int debug_intelli_plug;
...
static unsigned int nr_run_thresholds_balance[] = {
	...
	(THREAD_CAPACITY * 950 * MULT_FACTOR) / DIV_FACTOR,
	UINT_MAX
};
...
static unsigned int nr_run_thresholds_performance[] = {
	...
	(THREAD_CAPACITY * 720 * MULT_FACTOR) / DIV_FACTOR,
	UINT_MAX
};
...
static unsigned int nr_run_thresholds_conservative[] = {
	...
	(THREAD_CAPACITY * 2200 * MULT_FACTOR) / DIV_FACTOR,
	UINT_MAX
};
...
static void intelli_plug_work_fn(struct work_struct *work)
{
	...
	if (atomic_read(&intelli_plug_active) == 1)
		queue_delayed_work_on(0, intelliplug_wq,
			&intelli_plug_work, msecs_to_jiffies(def_sampling_ms));
}
					 ...
					 const char *buf, size_t count)
{
	...
	if (val == atomic_read(&intelli_plug_active))
		return count;

	intelli_plug_active_eval_fn(val);
...
static ssize_t show_down_lock_duration(struct kobject *kobj,
				       struct kobj_attribute *attr,
				       char *buf)
{
	return sprintf(buf, "%u\n", down_lock_dur);
...
static ssize_t store_down_lock_duration(struct kobject *kobj,
				        struct kobj_attribute *attr,
				        const char *buf, size_t count)
{
	int ret;
...
static int __init intelli_plug_init(void)
{
	...
	ret = sysfs_create_group(kernel_kobj, &intelli_plug_attr_group);
	if (ret) {
