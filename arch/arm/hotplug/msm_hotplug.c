...
/*
 * debug = 0 will completely disable debugging.
...
/*
 * Disable hotplugging during suspend.
 */
...
static int update_average_load(unsigned int cpu)
{
	...
	if (!pcpu->avg_load_maxfreq) {
		...
		/*
		 * There is a sample already available in this window.
...
static void online_cpu(unsigned int target)
{
	 ...
	 */
	if (target <= online_cpus || online_cpus >= hotplug.max_cpus_online)
		return;
...
static void offline_cpu(unsigned int target)
{
	 ...
	 */
	if (target >= online_cpus || online_cpus <= hotplug.min_cpus_online)
		return;
...
static void __ref msm_hotplug_suspend(void)
{
	...
	if (!hotplug.suspended) {
		...
		/*
		 * Do not leave cpu0 alone during suspend.
		 */
		cpu_up(1);
...
static int __ref msm_hotplug_start(void)
{
	...
	hotplug_wq = alloc_workqueue("msm_hotplug_wq",
	...
	if (!stats.load_hist) {
		pr_err("%s: Failed to allocate memory for load history\n",
							MSM_HOTPLUG);
		ret = -ENOMEM;
		goto err_dev;
	}
	...
	queue_delayed_work_on(0, hotplug_wq,
			&hotplug_work, msecs_to_jiffies(START_DELAY));
...
static DEVICE_ATTR(msm_enabled, 0644,
		   show_enable_hotplug, store_enable_hotplug);
static DEVICE_ATTR(down_lock_duration, 0644,
		   show_down_lock_duration, store_down_lock_duration);
static DEVICE_ATTR(boost_lock_duration, 0644,
		   show_boost_lock_duration, store_boost_lock_duration);
static DEVICE_ATTR(update_rates, 0644,
		   show_update_rates, store_update_rates);
static DEVICE_ATTR(load_levels, 0644,
		   show_load_levels, store_load_levels);
static DEVICE_ATTR(history_size, 0644,
		   show_history_size, store_history_size);
static DEVICE_ATTR(min_cpus_online, 0644,
		   show_min_cpus_online, store_min_cpus_online);
static DEVICE_ATTR(max_cpus_online, 0644,
		   show_max_cpus_online, store_max_cpus_online);
static DEVICE_ATTR(cpus_boosted, 0644,
		   show_cpus_boosted, store_cpus_boosted);
static DEVICE_ATTR(offline_load, 0644,
		   show_offline_load, store_offline_load);
static DEVICE_ATTR(fast_lane_load, 0644,
		   show_fast_lane_load, store_fast_lane_load);
static DEVICE_ATTR(fast_lane_min_freq, 0644,
		   show_fast_lane_min_freq, store_fast_lane_min_freq);
static DEVICE_ATTR(current_load, 0444,
		   show_current_load, NULL);
