...
static void __read_mostly *hw_mutex_reg_base;
...
node_init(struct device_node *node, phys_addr_t *reg_base, uint32_t *reg_size)
{
	...
	BUG_ON(ret || lock_count == 0);

	*reg_base = (phys_addr_t)res.start;
...
static void initialize_ops(void)
{
	...
	if (is_enabled(node) || IS_ENABLED(CONFIG_MSM_REMOTE_SPINLOCK_SFPB)) {
		...
		hardware_lock = true;
		pr_info("SFPB implementation was initialized\n");
	} else {
		...
		hardware_lock = false;
		pr_info("EX implementation was initialized\n");
	}
}
