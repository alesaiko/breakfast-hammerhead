...
static ssize_t gentle_fair_sleepers_show(struct kobject *kobj,
					 ...
					 char *buf)
{
	return scnprintf(buf, 3, "%u\n", !!sched_feat(GENTLE_FAIR_SLEEPERS));
}
					  ...
					  const char *buf, size_t count)
{
	...
	if (ret || enabled == !!sched_feat(GENTLE_FAIR_SLEEPERS))
		return -EINVAL;

	if (enabled) {
		sched_feat_set(__SCHED_FEAT_GENTLE_FAIR_SLEEPERS);
		sched_feat_enable(__SCHED_FEAT_GENTLE_FAIR_SLEEPERS);
	} else {
		sched_feat_rem(__SCHED_FEAT_GENTLE_FAIR_SLEEPERS);
